...
발목 잡히지 않게 했다. 둘 사이는 SharedState(락으로 보호)로만 통신한다.

⚠️ 카메라 3대는 일반 UVC 웹캠이 아니라 **Daheng Imaging MER2-240-159U3C (USB3 Vision 산업용
...
⚠️ 카메라 3대(좌/우/후방)의 실제 시리얼번호(CAMERA_SERIAL)는 이 저장소에 카메라가 없어
...
⚠️ CAMERA_CALIB_ID로 방향별 담당 캠 번호(left=1/right=2/rear=3)를 미리 확정해뒀다 — 설치 시
   이 번호로 캘리브레이션된 물리 카메라를 해당 방향에 붙일 것. 08_Video_Demo/calibration/ 참고.
...
실행:
    pip install ultralytics gxipy opencv-python Pillow
...
CAMERA_LABEL_KO = {"left": "좌측 카메라", "right": "우측 카메라", "rear": "후방 카메라(기본)"}
LOC_LABEL_KO = {"left": "좌측", "right": "우측", "rear": "후방"}
...
CAMERA_SERIAL = {"left": "", "right": "", "rear": ""}
CAMERA_INDEX_FALLBACK = {"left": 1, "right": 2, "rear": 3}  # gxipy는 index가 1부터 시작
_camera_handles = {}

CALIBRATION_DIR = REPO_ROOT / "08_Video_Demo" / "calibration"
# 방향별 담당 캠 번호를 미리 확정 — 설치 시 이 번호에 맞는 물리 카메라를 해당 방향에 붙이면 됨.
# cam4는 최종(10월) 4대 구성의 전방용으로 남겨두고 9/8 데모(좌/우/후방)에서는 사용하지 않음.
CAMERA_CALIB_ID = {"left": 1, "right": 2, "rear": 3}
_calib_cache = {}
...
def open_all_cameras():
    """좌/우/후방을 전부 미리 열어(stream_on) 둔다 — 전환 시 첫 프레임 지연이 없도록."""
    for camera in ("left", "right", "rear"):
...
def audio_worker(state: SharedState, panns_model, svm, class_names, tuning, detector, stream):
    ...
    while True:
        ...
        if top_class in CAMERA_TRIGGER_CLASSES:
            ...
            if camera == "front_no_camera":
                print(f"    -> doa={doa}deg: 전방 확인 (카메라 전환 없음)")
                state.trigger(None, top_class, "전방", None)
            else:
                detection = run_detection(detector, camera) if top_class in DETECTION_CLASSES else None
                print(f"    -> doa={doa}deg: {camera} 카메라로 전환, detection={detection}")
                state.trigger(camera, top_class, LOC_LABEL_KO[camera], detection)
...
def draw_overlay(frame_bgr, camera: str, banner, detection, show_front: bool, secondary):
    ...
    if banner is not None:
        ...
        put_text_kr(frame_bgr, f"{LEVEL_LABEL} → {banner['loc']}", (24 + 332, 46), 26, (0, 212, 255))
    ...
    if show_front:
        ...
        put_text_kr(frame_bgr, "전방 확인", (w // 2 - 160, h // 2 - 34), 56, (245, 245, 245))
